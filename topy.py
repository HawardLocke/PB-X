...
def gen_file(src_path, dst_path):
	...
	msg_id_str = msg_id_str + '\n\nvar %s = %s || {};\n\n%s.MsgID =\n{\n' % (settings.NAME_SPACE, settings.NAME_SPACE, settings.NAME_SPACE)
	file_index = 0
	os.chdir('.%s' % src)
	for file_name in os.listdir(abs_src_path):
		print('  %s' % file_name)
		os.system('..\\protoc.exe --py_out=import_style=commonpy,binary:../%s %s' % (dst_path, file_name))
	...
	os.chdir('./../')
	msg_id_str += '\n};\n'
	open(abs_dst_path+'MsgID.py', 'wb').write(msg_id_str)
pass
...
print('\n-----Begin Gen PY-----')
gen_file(src, gen_tmp)
print('-----End Gen PY-----')
if settings.AUTO_COPY_FILE:
	print('-----Begin Copy-----')
	copy_file(gen_tmp, settings.PY_DEST_DIR)
	print('-----End Copy-----\n')
pass
